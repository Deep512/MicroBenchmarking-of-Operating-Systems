...
int functioncall(int j1, int j2, int j3, int j4, int j5, int j6, int j7)
{
	return 7;
}
...
void main()
{
	...
	for (i = 0; i < LOOP_ITER; i++)
	{
		...
		//Code to measure
		return_value = functioncall(j1, j2, j3, j4, j5, j6, j7);
		asm volatile("rdtscp\n\t"
					 "mov %%edx, %0\n\t"
					 "mov %%eax, %1\n\t"
					 "cpuid\n\t"
	...
	printf("\nProcedure Call Overhead(Standard Deviation) : %lf\n", std_deviation);
}
