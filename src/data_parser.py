    ...
    for i in [1024, 2048, 16*1024, 32*1024, 64*1024, 128*1024, 256*1024, 512*1024, 1024*1024, 1024*1024*2, 1024*1024*3, 1024*1024*4, 1024*1024*8, 1024*1024*64, 1024*1024*128, 1024*1024*256]:
        ...
        for j in range(0, 10):
            output = commands.getoutput(
                BUILD_FOLDER + 'ram_access_measurement ' + str(i))
...
def measure_performance_measurement_overhead():
    output = commands.getoutput(
        BUILD_FOLDER + 'performance_measurement_overhead')
    print output
...
def measure_procedure_call():
    for i in range(0, 8):
        output = commands.getoutput(
            BUILD_FOLDER + 'procedure_call' + str(i) + '_measurement')
...
def measure_task_creation():
    output = commands.getoutput(
        BUILD_FOLDER + 'task_creation_fork_measurement')
    print output
    output = commands.getoutput(
        BUILD_FOLDER + 'task_creation_pthread_measurement')
    print output


def measure_context_switch():
    output = commands.getoutput(
        BUILD_FOLDER + 'context_switch_fork_measurement')
    print output
    output = commands.getoutput(
        BUILD_FOLDER + 'context_switch_pthread_measurement')
    print output
...
def measure_ram_bandwidth():
    ...
    for i in range(0, 10):
        output = commands.getoutput(
            BUILD_FOLDER + 'memory_read_bandwidth_measure')
    ...
    for i in range(0, 10):
        output = commands.getoutput(
            BUILD_FOLDER + 'memory_write_bandwidth_measure')
        clock_cycles = int(output.strip())
        result = 1.0/(clock_cycles*SECONDS_PER_CYCLE*pow(10, -9))
        bandwidth.append(result)
